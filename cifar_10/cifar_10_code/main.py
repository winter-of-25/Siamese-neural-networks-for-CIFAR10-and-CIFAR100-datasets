...
import os
import wandb
import torch._dynamo # 引入动态编译器配置
import timm 
...
def main():
    print(f"=== 🚀 SOTA CIFAR-10 Training (Auto-Save every 50 eps) ===")
    ...
    test_set = datasets.CIFAR10(root=Config.data_root, train=False, transform=test_transform, download=True)
    ...
    for epoch in range(Config.epochs):
        ...
        for batch_idx, (inputs, targets, caption_ids) in enumerate(train_loader):
            ...
            # 正常反向传播，生成 Scaled Grads
            scaler.scale(loss).backward()
            
            # 因为 SAM 的数学奇迹，这里不需要 Unscale，直接计算扰动 e_w
         ...
         # --- 核心修复：手动分离 SAM 还原 和 混合精度更新 ---
            # 1. 手动把模型权重从 e_w 的扰动中恢复回原位
            with torch.no_grad():  # [新增这一行！让梯度引擎闭嘴]
                for group in optimizer.param_groups:
                    for p in group["params"]:
                        if p.grad is not None and "e_w" in optimizer.state[p]:
                            p.sub_(optimizer.state[p]["e_w"])
                        
            # 2. 将干净的 base_optimizer 丢给 scaler
            # Scaler 会自动做两件事：Unscale(真实梯度) -> 检查NaN -> 如果正常则 base_optimizer.step()
